...
pub const Server = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, port: u16) !Server {
        ...
        const fcntl_res: c_int = std.c.fcntl(result.listener, std.c.F.SETFL, std.c.fcntl(result.listener, std.c.F.GETFL, 0) | std.posix.SOCK.NONBLOCK);
        if (fcntl_res == -1) {
            result.errno = std.posix.errno(-1);
                ...
                if (evt.data.fd == self.listener) {
                    ...
                    while (accept_running) {
                        ...
                        if (conn_sock == -1) {
                            ...
                            try self.manager.add_client(conn_sock);
                            errdefer self.manager.unsubscribe_all(conn_sock);
                            try self.poll.add_connection(conn_sock);
                    ...
                    while (read_running) {
                        const packet = reader.next_packet(self.alloc, evt.data.fd) catch |err| {
                            read_running = false;
                        ...
                        switch (packet.header.flags.opcode) {
                            ...
                            protocol.OpCode.c_close => {
                                const evt_fd = evt.data.fd;
                                self.manager.unsubscribe_all(evt_fd);
                                std.debug.print("closing connection for {}.\n", .{evt_fd});
                                self.poll.delete(evt_fd);
                                std.c.close(evt_fd);
                            },
